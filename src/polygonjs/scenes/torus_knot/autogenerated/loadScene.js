import { loadSceneData_torus_knot } from "./loadSceneData.js";

export const loadScene_torus_knot = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_torus_knot({ onProgress, sceneDataRoot })
      : (() => {
          return new Promise((resolve) => resolve());
        })(),
  ];
  const results = await Promise.all(promises);
  const { Poly, loadSceneFromSceneData_torus_knot } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_torus_knot({
    onProgress,
    sceneData,
    domElement,
    runRegister,
