import { loadSceneData_scene_sphere } from "./loadSceneData.js";

export const loadScene_scene_sphere = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_scene_sphere({ onProgress, sceneDataRoot })
      : (() => {
          return new Promise((resolve) => resolve());
        })(),
  ];
  const results = await Promise.all(promises);
  const { Poly, loadSceneFromSceneData_scene_sphere } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_scene_sphere({
    onProgress,
    sceneData,
    domElement,
    runRegister,
