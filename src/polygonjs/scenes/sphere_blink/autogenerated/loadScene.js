import { loadSceneData_sphere_blink } from "./loadSceneData.js";

export const loadScene_sphere_blink = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_sphere_blink({ onProgress, sceneDataRoot })
      : (() => {
          return new Promise((resolve) => resolve());
        })(),
  ];
  const results = await Promise.all(promises);
  const { Poly, loadSceneFromSceneData_sphere_blink } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_sphere_blink({
    onProgress,
    sceneData,
    domElement,
    runRegister,
